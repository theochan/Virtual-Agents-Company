...
function sourceHashes() {
  ...
  visit('src');
  files.push('server.ts', 'package.json', 'package-lock.json', 'scripts/evaluate-human-reviewed-packet.ts', 'scripts/evaluate-review-citation-regression.ts');
...
function intake() {
  ...
  for (const [f, hash] of Object.entries(prep.sourceHashesBeforeCaseCreation)) if (f !== 'src/server/semanticReview.ts' && sha(fs.readFileSync(f)) !== hash) throw Error('Unrelated pre-authoring source changed: ' + f);
...
if (mode === '--prepare') {
  const declaration = { sourceHashes: sourceHashes(), artifactHashes: Object.fromEntries([packetPath, labelsPath, preparationPath, baselineSummaryPath].map(f => [f, sha(fs.readFileSync(f))])), model: await modelIdentity(), endpoint, temperature: 0, maxTokens: 1536, timeoutMs: 90000, maximumCalls: 12,
    ...
    scope: 'Citation-responsibility candidate; six exposed Codex-authored, owner-adjudicated cases. Development regression only, NOT unseen qualification. Only semanticReview.ts may differ from the pre-authoring candidate snapshot; all current source and this driver frozen before inference. Original failed run hash-verified and preserved. No retries or tuning within this campaign.' };

...
const packetPath = 'docs/test-plans/fresh-review-packet-03.md';
const labelsPath = 'docs/evaluations/2026-09-23-rubric-review-human-adjudication.json';
const preparationPath = 'docs/evaluations/2026-09-23-rubric-review-preparation.json';
...
function sourceHashes() {
  ...
  visit('src');
  files.push('server.ts', 'package.json', 'package-lock.json', 'scripts/evaluate-rubric-review.ts');
...
async function modelIdentity() {
  ...
  if (item.digest !== prep.expectedModel.digest) throw Error('Model differs from pre-authoring pin');
  return { name: model, digest: item.digest };
}
function intake() {
  const prep = JSON.parse(fs.readFileSync(preparationPath, 'utf8'));
  for (const [f, hash] of Object.entries(prep.sourceHashesBeforeCaseCreation)) if (sha(fs.readFileSync(f)) !== hash) throw Error('Pre-authoring source changed: ' + f);
...
const cases = intake();
if (mode === '--prepare') {
  const declaration = { sourceHashes: sourceHashes(), artifactHashes: Object.fromEntries([packetPath, labelsPath, preparationPath].map(f => [f, sha(fs.readFileSync(f))])), model: await modelIdentity(), endpoint, temperature: 0, maxTokens: 1536, timeoutMs: 90000, maximumCalls: 12,
    cases: cases.map(c => ({ ...c, ...makePacket(c) })),
    acceptance: { maximumFalseAcceptances: 0, requiredTrueAcceptances: 3, maximumErrors: 0 },
    scope: 'Six Codex-authored rubric challenges, human labels required. Candidate and driver hash-pinned before case authoring. Correlated paired cases measure rubric distinction, not independent sample size or general qualification. One fixed campaign, no retries or tuning.' };

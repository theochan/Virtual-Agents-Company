...
const declaration={candidateStage:'Fresh synthetic end-to-end integration comparison; not independent human holdout',caseSelection:caseSelection||null,model,temperature:0,cases,maximumCalls:cases.length*42,
 ...
 scope:'One fresh synthetic structured-accounting task, one attempt per mode in fixed S/P/A order. Identical inputs and acceptance rules. Author-visible ground truth; not independent holdout, general extraction or demonstrated swarm advantage. Human correction time and monetary cost unmeasured.',
...
for(const [file,hash] of Object.entries(preparation.sourceHashes))if(sha(fs.readFileSync(file))!==hash)throw Error('Pre-authoring candidate changed: '+file);
...
try{
 ...
 for(const task of cases)for(const arm of task.order){
  ...
  try{
   ...
   result.checks={completed:run.status==='completed',currentArtifact:result.artifact?.runId===run.id&&result.artifact?.version===1,totals:!!content&&JSON.stringify(content.totals.map((t:any)=>[t.entity,t.currency,t.amount]))===JSON.stringify(task.totals),rows:content?.rows?.length===task.rows,duplicates:content?.rows?.filter((r:any)=>r.disposition==='duplicate').length===task.duplicates,contract:run.verification.length===1&&run.verification.every(c=>c.passed),reads:names.every(n=>readNames.includes(n)),semantic:run.semanticReviewResult?.passed===true,topology:run.nodes.length===(arm==='single'?1:3)};

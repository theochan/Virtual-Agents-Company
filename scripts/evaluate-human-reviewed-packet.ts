...
function sourceHashes() {
  ...
  visit('src');
  files.push('server.ts', 'package.json', 'package-lock.json', 'scripts/evaluate-human-reviewed-packet.ts');
...
if (mode === '--prepare') {
  const declaration = { sourceHashes: sourceHashes(), artifactHashes: Object.fromEntries([packetPath, labelsPath, preparationPath].map(f => [f, sha(fs.readFileSync(f))])), model: await modelIdentity(), endpoint, temperature: 0, maxTokens: 1536, timeoutMs: 90000, maximumCalls: 12,
    ...
    scope: 'Six Codex-authored, owner-adjudicated cases; first candidate exposure in this campaign. Not independently authored holdout or broad qualification. New harness frozen after authoring, candidate source verified unchanged since preparation. No retries or tuning.' };
    ...
    for (const c of d.cases) {
      ...
      try {
        for (const stage of ['assessment', 'confirmation']) {
          ...
          const response = await infer({ provider: 'ollama', model, endpoint, temperature: d.temperature, maxTokens: d.maxTokens, allowFinal: false }, packet.messages, AbortSignal.timeout(d.timeoutMs), [reviewToolFor(c.policy, c.packet.files)]);

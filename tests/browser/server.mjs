...
const provider = http.createServer(async (req, res) => {
  ...
  const decision = system.includes('VAC workflow planner') ? {action:'tool',toolId:'submit_workflow',parameters:{plan:[{key:'calculate',parentKey:'',agentId:'',name:'HarnessCalculator',role:'Calculator',instructions:'Use calculator once.',objective:'Calculate 19 plus 23',toolIds:['tool-calculator'],toolSequence:['tool-calculator'],requiredToolIds:['tool-calculator'],dependsOn:[],acceptanceCriteria:['42']}],toolSequence:[]}} : isSwarm ? nested ? nestedDecision : swarmDecision : instruction.includes('DELEGATE_BROWSER') && !childObserved ? { action: 'tool', toolId: 'tool-delegate', parameters: { subordinateId: subordinate.id, objective: 'CALCULATE_BROWSER', requiredToolIds: ['tool-calculator'] } }

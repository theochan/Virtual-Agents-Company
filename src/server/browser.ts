...
export const browserActionSchema = z.discriminatedUnion('action', [
  z.object({ action: z.literal('navigate'), url: z.string().url().max(2048) }).strict(),
...
export const browserTool = { id: BROWSER_TOOL, name: 'Isolated browser', schema: {type:'object',properties:{action:{type:'string',enum:['navigate','read','click','fill','select','press','scroll']},url:{type:'string'},selector:{type:'string'},value:{type:'string'},key:{type:'string',enum:['Enter','Tab','Escape','ArrowDown','ArrowUp']},direction:{type:'string',enum:['up','down']}},required:['action'],additionalProperties:false}, description: 'Navigate and read approved sites in your isolated session. Use CSS selectors from the returned page elements. Read mode click follows links only. fill/select/press and interactive clicks require owner-enabled actions. No arbitrary scripts, downloads, uploads, popups, private network or cookies from other agents. Every operation and network request consumes shared limits.' };
...
interface Session { profileKey?:string; projectId?:string; profileId?:string; browser: Browser; context: BrowserContext; page: Page; controller: AbortController; reserve: ()=>void; policy: BrowserPolicy; errors: string[] }
export class SwarmBrowser {
  ...
  async execute(id:string,raw:unknown,policy:BrowserPolicy,signal:AbortSignal,reserve:()=>void,projectId?:string) {
    ...
    if(args.action==='navigate')checkedUrl(args.url,policy);
    ...
    if(!s){
      ...
      if(profileKey&&this.profileDirectory){const f=path.join(this.profileDirectory,profileKey+'.json');if(fs.existsSync(f)){const saved=JSON.parse(fs.readFileSync(f,'utf8'));if(saved.expiresAt>Date.now())storageState=saved.state;else fs.rmSync(f);}}
        ...
        signal.throwIfAborted();
        const context=await browser.newContext({storageState,serviceWorkers:'block',acceptDownloads:false,permissions:[]});
        ...
        await context.route('**/*',async route=>{
          try{
            ...
            if(!['document','script','stylesheet','xhr','fetch'].includes(req.resourceType()))throw new Error('Browser resource type disabled');
            ...
            if(response.headers.location)checkedUrl(new URL(response.headers.location,req.url()).href,session.policy);
            await route.fulfill(response);
          }catch(error){session.errors.push(error instanceof Error?error.message:'Browser request blocked');session.errors=session.errors.slice(-8);await route.abort().catch(()=>{});}
        });
    ...
    s.reserve=reserve;s.policy=policy;
    ...
    }catch(error){await this.close(id);throw error;}finally{s.policy={...s.policy,allowActions:false};clearTimeout(timeout);signal.removeEventListener('abort',abort);}
  }
}

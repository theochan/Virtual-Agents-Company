...
export interface MemoryRecord {id:string;projectId:string;content:string;sourceRunId:string;sourceNodeId:string;status:'proposed'|'approved'|'rejected'|'superseded';createdAt:string;approvedAt?:string;subject?:string;supersedesId?:string;supersededBy?:string}
...
const memorySchema=z.object({query:z.string().max(300).optional(),limit:z.number().int().min(1).max(20).default(10),offset:z.number().int().min(0).max(200).default(0),propose:z.string().min(1).max(4000).optional(),subject:z.string().trim().min(1).max(80).regex(/^[a-zA-Z0-9][a-zA-Z0-9 ._/-]*$/).optional(),supersedesId:z.string().uuid().optional()}).strict().refine(a=>a.propose?true:!a.subject&&!a.supersedesId,'Subject and supersedesId are proposal fields');
...
export const WORKSPACE_TOOLS=[
 ...
 {id:'tool-memory',name:'Project memory',description:'Search approved project memory with bounded indexed retrieval and immutable run/node provenance, or propose a durable lesson with propose. Use an owner-meaningful subject when facts may change; supersedesId explicitly replaces an approved memory on owner approval. Conflicting current memories are withheld. Approval is historical evidence, not proof a claim is still current.',schema:memorySchema},
...
export class Workspace {
 ...
 decideMemory(id:string,status:'approved'|'rejected'){
  const m=this.store.get<MemoryRecord>('swarm-memory',id);if(!m)throw new HttpError(404,'Memory not found');this.project(m.projectId);if(m.status!=='proposed')throw new HttpError(409,'Memory was already reviewed');
  return this.store.transaction(()=>{
   if(status==='approved'&&m.supersedesId){const previous=this.store.get<MemoryRecord>('swarm-memory',m.supersedesId);if(!previous||previous.projectId!==m.projectId||previous.status!=='approved'||!m.subject||previous.subject!==m.subject)throw new HttpError(409,'Superseded memory must be an approved memory with the same project and subject');for(const replaced of this.memories(m.projectId).filter(memory=>memory.status==='approved'&&memory.subject===m.subject)){replaced.status='superseded';replaced.supersededBy=m.id;this.store.put('swarm-memory',replaced.id,replaced);}}
   m.status=status;if(status==='approved')m.approvedAt=now();this.store.put('swarm-memory',id,m);return m;
  });
 }
 retrieveMemory(projectId:string,query:string|undefined,limit:number,offset:number){
  this.project(projectId);
  const conflicts=this.store.memoryConflicts(projectId).map(conflict=>({...conflict,reason:'Multiple approved memories claim the same current subject; owner must approve an explicit superseding proposal.'}));
  const withheld=new Set(conflicts.flatMap(conflict=>conflict.memoryIds));
  const candidates=(query?this.store.searchMemory<MemoryRecord>(projectId,query,500):this.store.recentMemory<MemoryRecord>(projectId,500)).filter(memory=>!withheld.has(memory.id));
  const results=candidates.slice(offset,offset+limit).map(memory=>({...memory,freshness:{state:'owner_approved_historical',approvedAt:memory.approvedAt||null,warning:'Revalidate time-sensitive claims against current evidence.'}}));
  return{query:query||null,offset,limit,returned:results.length,hasMore:candidates.length>offset+limit,results,withheldConflicts:conflicts,withheldSuperseded:this.store.countMemory(projectId,'superseded'),provenance:'Every result retains sourceRunId, sourceNodeId and createdAt; approval does not make it current fact.'};
 }
 ...
 async execute(j:SwarmJob,nodeId:string,toolId:string,raw:unknown,callId:string,signal:AbortSignal){
  ...
  if(toolId==='tool-files'){
   ...
   const a=memorySchema.parse(raw);if(a.propose){if(a.supersedesId){const previous=this.store.get<MemoryRecord>('swarm-memory',a.supersedesId);if(!previous||previous.projectId!==j.projectId||previous.status!=='approved'||!a.subject||previous.subject!==a.subject)throw new Error('supersedesId requires an approved memory with the same project and subject');}const m:MemoryRecord={id:uid(),projectId:j.projectId,content:a.propose,sourceRunId:j.id,sourceNodeId:nodeId,status:'proposed',createdAt:now(),...(a.subject?{subject:a.subject}:{}),...(a.supersedesId?{supersedesId:a.supersedesId}:{})};this.store.put('swarm-memory',m.id,m);output=m;}else output=this.retrieveMemory(j.projectId,a.query,a.limit,a.offset);

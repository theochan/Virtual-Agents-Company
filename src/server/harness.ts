...
class GuardedModel extends BaseChatModel {
  ...
  bindTools(tools:any[]):any{return new GuardedModel(this.dispatch,tools.filter(t=>['submit_workflow','write_todos'].includes(t.name)&&this.canUse(t.name)),this.finished,this.canUse);}
...
export async function planWithHarness(options:{
  ...
  let accepted:any;let calls=0;let todoWritten=false;
  ...
  },[],()=>!!accepted,name=>name!=='write_todos'||!todoWritten);
  ...
  const guard=createMiddleware({name:'VACPlanningBoundary',wrapToolCall:async(request,handler)=>{
    options.signal.throwIfAborted();
    if(!['submit_workflow','write_todos'].includes(request.toolCall.name))throw new Error('Harness tool is outside planning authority');
    if(request.toolCall.name==='write_todos'){if(todoWritten)throw new Error('Planning todo list already recorded');todoWritten=true;}
  ...
  const agent=createDeepAgent({model,tools:[submit],systemPrompt:options.prompt,middleware:[
    ...
    todoListMiddleware(),guard,
  ]});
  const result=await agent.invoke({messages:[{role:'user',content:'Create the workflow now. You may record a concise todo list, then call submit_workflow. Do not execute the work or answer with an unsubmitted plan.'}]} as never,{signal:options.signal,recursionLimit:16,callbacks:[]});

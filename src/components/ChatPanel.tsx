...
export const ChatPanel: React.FC<ChatPanelProps> = ({ agent, projects, activeProject, messages, onSendMessage, isSending,
  onOpenProfile, onOpenArtifact, onUpdateAgentLLM, activeTask, artifacts = [], memories = [], onSelectProject }) => {
  const [input, setInput] = useState('');
  const bottom = useRef<HTMLDivElement>(null);
  useEffect(() => { const container = bottom.current?.parentElement; if (container) container.scrollTo({ top: container.scrollHeight, behavior: 'smooth' }); }, [messages]);
  ...
  return <div className="flex flex-1 min-w-0 h-full">
    <section className="flex flex-col flex-1 min-w-0">
      <header className="bg-white border-b p-5 space-y-3">
        ...
        <p className="text-xs text-slate-500">Answers are drafts. Review tool receipts and accept deliverables in Runs and approvals.</p>
      </header>
      <div className="flex-1 overflow-auto p-6 space-y-5">
        ...
        {messages.map(message => <article key={message.id} className={`max-w-3xl rounded-xl p-4 border ${message.senderType === 'user' ? 'ml-auto bg-amber-50 border-amber-100' : message.senderType === 'system' ? 'bg-red-50 border-red-200' : 'bg-white border-slate-200'}`}>
          ...
          {message.metadata?.executionStatus && <p className="text-xs font-semibold text-amber-800 mb-2">{message.metadata.executionStatus.replaceAll('_', ' ')}</p>}
        ...
        {isSending && <p role="status" className="text-sm text-slate-600">Run {activeTask?.status || 'being submitted'}. Open Runs and approvals to inspect progress or cancel.</p>}
        <div ref={bottom} />
      </div>
      <form className="border-t bg-white p-4 flex gap-3" onSubmit={async event => { event.preventDefault(); if (!input.trim() || isSending) return; const message = input; await onSendMessage(message); setInput(''); }}>

...
export const ChatPanel: React.FC<ChatPanelProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col h-screen bg-[#050505] text-[#E0E0E0] overflow-hidden">
      ...
      {/* Autonomous Workstream Active Banner */}
      <div className="px-6 py-2 bg-emerald-950/20 border-b border-emerald-900/30 flex items-center justify-between text-xs shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-medium text-emerald-300">
            Autonomous Workstream Active: Task {agent.displayName} with any work in chat — items will auto-create on the <strong className="text-white font-mono">{activeProject?.name || 'Project'}</strong> Kanban board and close upon completion.
          </span>
        </div>
      </div>

      {/* Quick Tasking & Scenario Triggers Bar */}
      <div className="px-6 py-2.5 bg-[#070707]/60 border-b border-[#1A1A1A] flex items-center gap-2 overflow-x-auto text-xs shrink-0">
        ...
        {agent.id === 'agent-marcus' ? (
          <>
            <button
              onClick={() =>
                onSendMessage('Marcus, implement the Redis session caching adapter and run connection pool benchmarks.')
              }
            ...
            >
              &quot;Task: Implement Redis caching adapter & connection pool&quot;
            </button>
            <button
              onClick={() =>
                onSendMessage('Marcus, write schema migration scripts for zero-downtime column partition.')
              }
              className="px-2.5 py-1 rounded border border-[#1A1A1A] bg-[#0A0A0A] hover:border-[#C5A358]/40 text-[#BBB] hover:text-[#C5A358] text-[11px] whitespace-nowrap transition cursor-pointer"
            >
              &quot;Task: Write zero-downtime migration scripts&quot;
            </button>
          </>
        ) : agent.id === 'agent-emma' ? (
          <>
            <button
              onClick={() =>
                onSendMessage('Emma, conduct an enterprise benchmark of pgvector vs Milvus for semantic document indexing.')
              }
              className="px-2.5 py-1 rounded border border-[#1A1A1A] bg-[#0A0A0A] hover:border-[#C5A358]/40 text-[#BBB] hover:text-[#C5A358] text-[11px] whitespace-nowrap transition cursor-pointer"
            >
              &quot;Task: Benchmark pgvector vs Milvus indexing latency&quot;
            </button>
            <button
              onClick={() =>
              ...
              className="px-2.5 py-1 rounded border border-[#1A1A1A] bg-[#0A0A0A] hover:border-[#C5A358]/40 text-[#BBB] hover:text-[#C5A358] text-[11px] whitespace-nowrap transition cursor-pointer"
            >
              &quot;Check Project Memory: Database decision & rationale&quot;
            </button>
          </>
        ) : agent.id === 'agent-daniel' ? (
          <>
            <button
              onClick={() =>
                onSendMessage('Daniel, audit our Q4 infrastructure cloud spend and model multi-region egress costs.')
              }
              className="px-2.5 py-1 rounded border border-[#1A1A1A] bg-[#0A0A0A] hover:border-[#C5A358]/40 text-[#BBB] hover:text-[#C5A358] text-[11px] whitespace-nowrap transition cursor-pointer"
            >
              &quot;Task: Audit Q4 cloud spend & egress cost model&quot;
            </button>
          </>
        ) : agent.id === 'agent-ava' ? (
          <>
            <button
              onClick={() =>
                onSendMessage('Ava, design the high-density executive analytics dashboard UI specs and component tokens.')
              }
              className="px-2.5 py-1 rounded border border-[#1A1A1A] bg-[#0A0A0A] hover:border-[#C5A358]/40 text-[#BBB] hover:text-[#C5A358] text-[11px] whitespace-nowrap transition cursor-pointer"
            >
              &quot;Task: Design executive analytics UI specs & tokens&quot;
            </button>
          </>
        ) : isLeadAgent ? (
          <>
            <button
              onClick={() =>
                onSendMessage('Sarah, start a task to build the enterprise multi-tenant architecture across engineering. Break down into Backlog, verify Atlas busy status, and execute sequentially.')
              }
              className="px-2.5 py-1 rounded border border-purple-500/50 bg-purple-950/20 hover:bg-purple-950/40 text-purple-300 text-[11px] whitespace-nowrap transition cursor-pointer flex items-center gap-1.5 font-medium"
            >
              <Layers className="w-3 h-3 text-purple-400" />
              <span>&quot;Start task: Break down to Backlog, wait on Atlas busy tasks & execute sequentially&quot;</span>
            </button>
            <button
              onClick={() =>
                onSendMessage('Sarah, retrieve PostgreSQL 17 and pgvector benchmarks data from the internet.')
              }
              className="px-2.5 py-1 rounded border border-[#C5A358]/50 bg-[#C5A358]/10 hover:bg-[#C5A358]/20 text-[#C5A358] text-[11px] whitespace-nowrap transition cursor-pointer flex items-center gap-1.5 font-medium"
            >
              <Globe className="w-3 h-3 text-[#C5A358]" />
              <span>&quot;Retrieve PostgreSQL 17 & pgvector data from the internet&quot; (Delegates to Emma)</span>
            </button>
            <button
              onClick={() =>
                onSendMessage('Sarah, coordinate a security review and customer SLA compliance plan for enterprise tenants.')
              }
              className="px-2.5 py-1 rounded border border-[#1A1A1A] bg-[#0A0A0A] hover:border-[#C5A358]/40 text-[#BBB] hover:text-[#C5A358] text-[11px] whitespace-nowrap transition cursor-pointer"
            >
              &quot;Task: Coordinate security review & customer SLA compliance plan&quot;
            </button>
            <button
              onClick={() =>
                onSendMessage('Sarah, break down the Q1 engineering roadmap into assigned specialist work packages.')
              }
              className="px-2.5 py-1 rounded border border-[#1A1A1A] bg-[#0A0A0A] hover:border-[#C5A358]/40 text-[#BBB] hover:text-[#C5A358] text-[11px] whitespace-nowrap transition cursor-pointer"
            >
              &quot;Task: Break down roadmap into specialist work packages&quot;
            </button>
          </>
        ) : (
          <button
            onClick={() =>
              onSendMessage(`Execute an audit of our active deliverable criteria and verify readiness for deployment.`)
            }
            className="px-2.5 py-1 rounded border border-[#1A1A1A] bg-[#0A0A0A] hover:border-[#C5A358]/40 text-[#BBB] hover:text-[#C5A358] text-[11px] whitespace-nowrap transition cursor-pointer"
          >
            &quot;Task: Audit deliverable criteria & verify readiness&quot;
          </button>
        )}
      </div>

...
export function RunReviewView({ runs, approvals, onAction, onDecide, onSelectAgent }: {
  ...
  return <div className="flex-1 p-6 overflow-auto bg-slate-50 space-y-4">
    ...
    <p className="text-sm text-slate-600">Root drafts require your acceptance. Delegated child outputs are evidence for their parent, not accepted business work. Tool approval authorizes the displayed operation; it does not certify the content.</p>
    ...
    {[...runs].reverse().map(run => <article id={`run-${run.id}`} key={run.id} className="border rounded-xl bg-white p-5 space-y-3">
      ...
      {run.status === 'reviewing' && !(run as any).parentRunId && <div className="flex gap-2">

...
The optional `plan` field supplies up to 16 named worker specifications. Each needs a unique `key`; `parentKey` refers to its supervisor (omitted means the root). `dependsOn` refers to prerequisite keys anywhere in the same plan, including other branches. Dependencies belong on leaf workers; supervisors wait for their children. Validation includes those implicit supervisor waits, rejecting ancestor dependencies and cross-branch completion cycles before any nodes persist. Dynamic `tool-spawn-agent` batches retain their sibling-only dependency scope. Each step supplies the same role, instructions, objective, tools and acceptance fields as a dynamic spawn, plus optional `agentId` and `requiredToolIds`. Optional `toolSequence` lists up to 24 operations in the required order; a tool ID may occur more than once for separate reads, calculations or writes. Only the next occurrence is offered until it succeeds, and its receipt carries a zero-based `sequenceStep`. A prior occurrence cannot satisfy a later one. Successful identical side effects cannot replay; write approvals and all existing budgets remain enforced. Root requests can also supply a coordinator `toolSequence`. This is an owner-authored execution contract, not autonomous planning.
...
## Research workflow acceptance controls (2026-09-22)

`workflowRequirements` accepts `tasks: [{id, briefName?, minimumTools, dependsOn}]` and `coordinatorMinimumTools`. Counts refer to distinct sequence occurrences, and dependencies refer to assignment IDs. Compiled workers retain assignment identity even when their display names differ. Requirements need a generated or owner-authored plan, cannot expand tool authority, and are preserved in saved templates. Owner briefs are frozen when a run is created, not reloaded as mutable instructions during execution.

Browser `navigate` and `read` accept an optional bounded `focus` string; `read` also accepts a nonnegative text `offset`. Returned text is capped at 9,000 characters, with offset, total length, truncation and focus-match metadata. Existing origin, network, request and action limits still apply. A focus match only locates text; it does not verify its accuracy.

`scripts/evaluate-ticker-research.ts` runs the MSFT development scenario against the configured local model and Tavily provider in an isolated workspace. Request reservations share the owner's durable daily counters. It saves immutable attempt evidence and current-run artifacts, checks schemas/provenance/arithmetic, and always leaves independent factual acceptance pending. Synthetic validator fixtures do not constitute a successful live investment recommendation.

For research workflows, `browserPolicy.requireDiscoveredUrls: true` additionally restricts explicit navigation to approved URLs from successful search/browser receipts in the current run. The model receives a bounded navigation URL enum; the browser enforces the full discovered set before launching or sending a request. Returned DOM links and guarded redirect destinations remain subject to exact origin checks. This constraint prevents invented paths; it does not certify the contents of discovered pages. An empty discovered set offers no navigation operation.

`browserPolicy.documentOnly: true` is an opt-in research mode: JavaScript is disabled and only document resources are fetched. It rejects interactive actions and cannot be switched within a browser session. Dynamic-only content remains unavailable; missing facts must be disclosed. Approved GET redirects remain guarded and count individually. Request-budget exceptions retain their original identity through routing. The ticker evaluator uses this mode and checks the remaining daily model allowance before starting; a full trial needs at least 57 calls, and an actual generated plan may need more.

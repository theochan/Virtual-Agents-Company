...
| Reviewed skill capture | Completed-run trace drafts, explicit parameter paths, owner approval, changed-input replay, version diff and immutable rollback |
...
- Imported execution is deny-by-default: `tool-code` accepts the hash-pinned `meeting-cost-v1` adapter with strict numeric/boolean arguments, checked output and versioned provenance. Its seven deterministic cases and hostile-input/container checks passed; the other 778 inventoried scripts remain unreviewed and disabled through this path. General owner-granted sandbox code remains separately available. See [qualification evidence](docs/reviews/2026-09-22-imported-tools.md) and [inventory](docs/imported-tool-inventory.json). This is one qualified tool, not blanket approval of the vendor library (VAC-37).
...
- The [VAC-40 engineering license audit](docs/reviews/2026-09-24-vendored-licensing.md) inventories the pinned vendor snapshot, exact locked dependencies, preserved license texts and remote-only portrait references; build/publication fail on drift or unknown licenses. This is an auditable redistribution gate, not legal advice or legal certification. [VAC-41’s dated Git-history and release audit](docs/reviews/2026-09-22-secret-audit.md) found no confirmed exposure; it is not exhaustive or permanent absence certification.
